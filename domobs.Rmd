...
```{r David Score ranks within groups and plotting to get steepness 1, include=F}
...
N=length(colnames(results))

slopes$p <- sapply(1:N, function(i) {
  mean(results[[i]] < slopes$Slopes[i])
})
...
ggplot(results)+
  ...
  geom_text(data = slopes, aes(y = 20, x = -.6, 
                               label = ifelse(p == 0, 
                               "p<0.001", 
